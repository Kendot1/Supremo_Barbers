...
export function AnalyticsOverview({ onNavigateToPaymentVerification, onNavigateToBookings }: AnalyticsOverviewProps) {
  ...
  const analytics = useMemo(() => {
    ...
    const revenueData = Array.from(revenueByMonth.entries()).map(([month, data], index) => ({
      month,
      revenue: data.revenue,
      customers: data.customers.size,
      bookings: data.bookings,
      id: `${month}-${index}`, // Add unique id
    }));
    ...
    const serviceDistribution = Array.from(serviceCount.entries())
      .map(([name, count], index) => ({
        ...
        id: `service-${name}-${index}`, // Add unique id
      }))
    ...
    const allPendingPayments = appointments.filter(apt => {
      const paymentStatus = apt.payment_status || apt.paymentStatus;
      return paymentStatus === 'pending';
    }).length;

    const allVerifiedPayments = appointments.filter(apt => {
      const paymentStatus = apt.payment_status || apt.paymentStatus;
      return paymentStatus === 'verified' || paymentStatus === 'paid';
    }).length;
    ...
    const upcomingBookings = appointments
      .filter(apt => {
        ...
        const isValidStatus = apt.status !== 'completed' && apt.status !== 'cancelled' && apt.status !== 'rejected';
      ...
      .map(apt => {
        ...
        return {
          ...
          status: apt.status,
          payment: paymentStatus === 'verified' || paymentStatus === 'paid' ? 'verified' : 'pending',
        };
      });
  ...
  const { revenueData, serviceDistribution, dailyBookings, recentTransactions, upcomingBookings, pendingPayments, verifiedPayments, totalTransactions } = analytics;
          ...
          <CardHeader>
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <CardTitle className="text-[#5C4A3A]">Upcoming Bookings</CardTitle>
              </div>
          ...
          <CardContent className="space-y-4 flex-1 flex flex-col">
            ...
            {upcomingBookings.length > 0 ? (
                  ...
                  {upcomingBookings.map((booking) => (
                    ...
                    >
                      <div className="flex-1">
                        <div className="flex items-center gap-2">
                          <span className="text-sm font-medium text-[#5C4A3A]">{booking.customer}</span>
                          <Badge 
                            variant="outline" 
                            className={`text-xs ${
                        ...
                        <p className="text-xs text-[#87765E] mt-1">{booking.service}</p>
                      </div>
                      <div className="text-right ml-2">
                        <p className="text-sm text-[#DB9D47]">{booking.date}</p>
                        <p className="text-xs text-[#87765E]">{booking.time}</p>
                      </div>
                    </div>
                  ))}
                </div>
              ...
              <div className="flex-1 flex items-center justify-center py-8">
                <p className="text-sm text-[#87765E]">No upcoming bookings</p>
              </div>
            )}
