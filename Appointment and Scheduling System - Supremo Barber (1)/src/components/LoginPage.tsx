  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.user.role === 'customer') {
        ...
        // Fire-and-forget: notify the customer + admin about a new-device login (non-blocking)
        logNewDeviceLoginWithAdminAlert(
          response.user.id,
          response.user.name,
          response.user.email.toLowerCase(),
          response.user.role as 'customer' | 'barber' | 'admin',
          {
